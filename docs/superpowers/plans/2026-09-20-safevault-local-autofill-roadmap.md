...
默认 15 分钟；设置允许 `5/15/30/60` 分钟；窗口隐藏不立即锁定，Windows 会话锁定和应用退出必须立即锁定。
...
| 2026-09-21 | Task 12 | 未开始 → 进行中 | `npm run verify` 全部通过；v3.3.4 自包含 Bridge、NSIS 和便携版构建成功且资源哈希一致；因 `D:\APP\密码管理` 旧版实例正在运行，临时 NSIS/便携版启动验收被单实例锁拒绝，未关闭用户程序，密码库哈希保持不变 |
